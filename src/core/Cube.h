...
#define CUBE_SIZE             0x04
#define CUBE_BYTE_SIZE        CUBE_SIZE * CUBE_SIZE
#define CUBE_SIZE_MASK        0x03
#define CUBE_BYTE_SIZE_MASK   0x0f
...
public:
    ...
    inline unsigned char* getFrontBuffer() {
        return frontBuffer;
    }
     ...
     */
    void copyBuffer(unsigned char copyDirection);

    /**
     * Set if will use back or front buffer to write to.
     */
    void selectBufferType(Buffer buffer);

    /**
     * Set the buffer to use back buffer.
     */
    inline void useDoubleBuffer() {
        selectBufferType(DOUBLE_BUFFER);
    }

    /**
     * Set the buffer to use front buffer.
     */
    inline void useSingleBuffer() {
        selectBufferType(SINGLE_BUFFER);
    }
     ...
     */
    inline void writeVoxel(Point *p, Voxel v) {
